    ...
    procesarGridEmpresas:function(nombreGrid, accion) {
        ...
        grid.jqGrid({
            ...
            postData: {
            	...
            	numeroExpediente: $('#txtNroExpediente').val()
            },
            hidegrid: false,
	...
	actaInspeccion : function(cellvalue, options, rowdata) {
		...
		if (numeroExpediente != null) {
			 if (nombre != null && nombre != '' && idOficioDoc!='' && idOficioDoc!=null){       
		            html = '<a href="#" class="link" style="color:#0073ea;" onClick="modalActasExpedientes.actasExpedientes(\''+numeroExpediente +'\');" >'+
		            ' Ver '+
		            '</a>';
		        }
		}
			...
			$.ajax({
				...
				success : function(data) {
					...
					$("#dialogActaExpedientesZonaEspecialista").dialog({
						...
						closeText : "Cerrar",
						close:function(){
	                           //$("#dialogActaExpedientesZonaEspecialista").dialog("destroy");
	                           //$(this).remove();
//							$(this).dialog("destroy");
//							$(this).remove();
	                    }
...
$(function() {
	...
	$("#txtNroExpediente").alphanum(constant.valida.alphanum.soloNumeros);
