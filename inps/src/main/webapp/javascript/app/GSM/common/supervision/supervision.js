...
var coSupeSupe = (function() {
    ...
    function comportamientos() {
        ...
        $('#btnLimpiar').click(function() {
            confirmer.open('¿Confirma que desea limpiar la b&uacute;squeda realizada?', 'coSupeSupe.limpiarObligacionBusqueda()');
        });
        ...
        $('#radioNoSupervision').click(function() {
            ...
            confirmer.open('Al realizar esta operaci&oacute;n se perder&aacute;n los datos ingresados.<br>¿Confirma que no se realiz&oacute; la supervisi&oacute;n de la orden de servicio: ' + $('#txtNumeroOrdenServicio').val() + ' ?', 'coSupeSupe.ejecutarNoSupervision()');
        });
        //check
    ...
    function grillaObligaciones() {
        ...
        if ($('#idSupervision').val() != '' && $('#idSupervision').val() != null) {
            ...
            var nombres = ['#', 'idDetalleSupervision', 'idDetalleSupervisionAnt', 'obligacion.idObligacion', 'supervision.idSupervision', 'tipificacion.idTipificacion', 'criterio.idCriterio', 'flagResultado', 'flagRegistrado',
                'descripcionResultado', 'C&Oacute;D. OBLIGACI&Oacute;N', 'OBLIGACI&Oacute;N NORMATIVA', 'BASE LEGAL', 'ADJ.', 'VER HALLAZGO', 'SI', 'NO', 'SI', 'NO', 'VER', 'VER', ''];
            var columnas = [
    ...
    function guardarDatosSupervision() {
        ...
        if (validarFormularioDatosSupervision()) {
            ...
            if (mensajeValidacion == '') {
                confirmer.open('¿Confirma que desea registrar los datos de supervisi&oacute;n?', 'coSupeSupe.registrarDatosSupervision()');
            }
            else {
                mensajeGrowl('warn', mensajeValidacion);
            }
    ...
    function guardarPersonaAtiende() {
        ...
        if (validarFormularioPersonaAtiende()) {
            ...
            if (mensajeValidacion == '') {
                confirmer.open('¿Confirma que desea registrar los datos de la persona que atiende la visita?', 'coSupeSupe.registrarPersonaAtiende()');
            }
            else {
                mensajeGrowl('warn', mensajeValidacion);
            }
    ...
    function registrarDatosSupervision() {
        ...
        $.getJSON(baseURL + "pages/supervision/guardarDatosSupervision",
                {
                    idSupervision: $('#idSupervision').val(),
                    ...
                    observacion: $('#txtObservacionRS').val().toUpperCase(),
    ...
    function registrarPersonaAtiende() {
        ...
        $.getJSON(baseURL + "pages/supervision/guardarPersonaAtiende",
                {
                    idSupervision: $('#idSupervision').val(),
    ...
    function desbloquearModoConsultaGrillaObligacion(iteracion) {
        if (constant.iteracion.primera == iteracion) {
            ...
            $("#gridObligaciones").jqGrid('setGroupHeaders', {
                ...
                groupHeaders: [
                    {startColumnName: 'cumple', numberOfColumns: 2, titleText: '¿CUMPLE?'}
                ]
            });
        } else if (constant.iteracion.segunda == iteracion) {
            ...
            $("#gridObligaciones").jqGrid('setGroupHeaders', {
                useColSpanStyle: true,
                groupHeaders: [
                    {startColumnName: 'sanciona', numberOfColumns: 2, titleText: '¿SE SANCIONA?'}
                ]
            });
        }
    }
