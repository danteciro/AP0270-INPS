...
* OSINE_MANT_DSR_003 03/07/2017  |   Oliver Quispe Huarcaya::ADAPTER |Agregar la funcionalidad "consultar" en la tabla de datos NOTIFICADO/ARCHIVADO(pestania) 
*/
...
//            {name: "empresaSupervisada.nroIdentificacion", hidden:true},
        ...
        grid.jqGrid({
            ...
            onRightClickRow: function(rowid) {
            	/* OSINE_MANT_DSR_003  -  INICIO */
            	$('#linkConsultarExpeNotif').attr('onClick', 'common.ordenServicio.abrirConsultar("' + rowid + '","gridNotificadoArchivado")');
                var row=$('#'+"gridNotificadoArchivado").jqGrid("getRowData",rowid); 
                if(row['ordenServicio.idPeticion.codigo']!=constant.codTipoPeticion.anular){
                	$('#linkAnularExpeNotif').css('display','none');
                }else{
                	$('#linkAnularExpeNotif').css('display','');
                } 
                if(row['ordenServicio.idPeticion.codigo']!=constant.codTipoPeticion.editar){ 
                	$('#linkEditarExpeNotif').css('display','none');
                }else{
                	$('#linkEditarExpeNotif').css('display','');
                } 
                $('#linkAnularExpeNotif').attr('onClick', 'common.abrirOrdenServicio("'+constant.accionOS.anular+'","' + rowid + '","gridNotificadoArchivado")'); 
                $('#linkEditarExpeNotif').attr('onClick', 'common.abrirOrdenServicio("'+constant.accionOS.editar+'","' + rowid + '","gridNotificadoArchivado")'); 
...
//                /* OSINE_SFS-480 - RSIS 03 - Fin */
            ...
            loadComplete: function(data) {
                ...
                $('#divContextMenuNotificadoArchivado').html("<ul id='contextMenuNotificadoArchivado'>"
                        + "<li> <a id='linkConsultarExpeNotif' data-icon='ui-icon-search' title='Consultar'>Consultar</a></li>"
                        + "</ul>");
                $('#contextMenuNotificadoArchivado').puicontextmenu({
                    target: $('#gridNotificadoArchivado')
                });
                $('#contextMenuNotificadoArchivado').parent().css('width','190px');
