...
//common/OrdenServicio/OrdenServicioAtender.js
var coOrSeOrSeAte = {
		...
		procesaAtenderOrdenServicio : function() {
			...
			$.getJSON(
					baseURL + "pages/ordenServicio/atender",
					{
						...
						'ordenServicio.idOrdenServicio' : $('#txtIdOrdenServicioOSAte').val(),
						...
						/* OSINE791 RSIS42 - Inicio */
//						'empresaSupervisada.idEmpresaSupervisada' : $('#txtIdEmpresaSupervisadaOSAte').val(),
						'flujoSiged.codigoSiged' : $('#slctIdFlujoSigedOSAte').find(':checked').attr('codigoSiged'),
                        ...
                        'unidadSupervisada.ruc':$('#txtRucUnidadSupervisadaOSAte').val(),
                        'unidadSupervisada.codigoOsinergmin':$('#txtCodigoOsineOSAte').val(),
                        'proceso.idProceso':$('#txtIdProcesoAte').val(),
                        /* OSINE_SFS-Ajustes - mdiosesf - Inico */
                        'ordenServicio.numeroOrdenServicio': $('#txtNumeroOrdenServicioOSAte').val()
	...
	fillDataUnidadOperativa : function(data) {
            ...
            if (data != null && data.registro!=null) {
                ...
                if(data.registro.direccionUnidadsupervisada.direccionCompleta!=null && data.registro.direccionUnidadsupervisada.direccionCompleta!=undefined){
                    ...
                    $('#txtIdDirccionUnidadSuprvisadaOSAte').val(data.registro.direccionUnidadsupervisada.idDirccionUnidadSuprvisada); 
                }
            }
	},
	...
	$('#btnSubirDocumentoAte').click(
            function() {
                common.abrirSubirDocumento(
                    $('#txtNumeroExpedienteOSAte').val(),
                    $('#txtIdOrdenServicioOSAte').val(), 
//                    $('#txtIdEmpresaSupervisadaOSAte').val(), 
                    $('#txtIdExpedienteOSAte').val(), 
                    $('#txtAsuntoSigedOSAte').val(), 
                    $('#slctIdFlujoSigedOSAte').find(':checked').attr('codigoSiged'),
                    $('#txtIdDirccionUnidadSuprvisadaOSAte').val(),
                    $('#slctUnidSupeOSAte').val()
                );
            });
