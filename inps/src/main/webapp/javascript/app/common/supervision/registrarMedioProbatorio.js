...
var coSuReMePro = {
    validarAgregarMedioProbatorio : function(){
        ...
        if($('#formMedioAprobatorioDsr').validateAllForm('#divMensajeValidaFrmMDDsr')){
            if(mensajeValidacion==''){
            	if(!coSuReMePro.validarExisteArchivo()){
    ...
    validarExisteArchivo:function(){
    	var existe = false;
        var url = baseURL + "pages/supervision/dsr/validarExisteArchivo";
        $.ajax({
            url: url,
            type: 'post',
            async: false,
            data: { 
            	...
            	nombreArchivo:$('#file_nameSuperDsr').val()},
